...
class CommunicationStation:
    ...
    def update_diode(self, probe_id, connected):
        if connected:
            self.diode_canvas[probe_id].config(bg='green')
        else:
            self.diode_canvas[probe_id].config(bg='red')

    def update_server_diode(self, running):
        if running:
            self.server_diode_canvas.config(bg='green')
        else:
            self.server_diode_canvas.config(bg='red')
    ...
    async def connect_all_probes(self):
        for probe_id in self.probes.keys():
            await self.connect_probe(probe_id)

    async def disconnect_all_probes(self):
        for probe_id in self.probes.keys():
            await self.disconnect_probe(probe_id)

    def update_temperature(self, probe_id, temperature):
        self.temperature_labels[probe_id].config(text=f"Last Temp: {temperature:.2f}°C")
    ...
    def insert_message(self, message):
        ...
        if len(self.messages_text[self.message_index].get(1.0, tk.END)) > 2000:  # Example size limit for switching column
            self.message_index += 1

if __name__ == "__main__":
    root = tk.Tk()
    data_processor = None  # Replace with actual data_processor instance
    data_storage = None  # Replace with actual data_storage instance
